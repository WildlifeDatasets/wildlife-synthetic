...
def parse_args():
    ...
    parser.add_argument('--hf_key', type=str, default=None,
                        help='Hugging Face model repo ID or local path; overrides --sd_version.')
    parser.add_argument('--sd_local_files_only', action='store_true',
                        help='Load SD weights only from local cache/path, no network calls.')
    parser.add_argument('--sd_cache_dir', type=str, default=None,
                        help='Optional cache dir for Hugging Face models.')
    parser.add_argument('--hf_token', type=str, default=None,
                        help='Optional Hugging Face token for gated/private models.')
    parser.add_argument('--disable_sd_fallback', action='store_true',
                        help='Disable automatic fallback from requested SD model to SD 1.5.')
    ...
    parser.add_argument('--n_iter', type=int, default=3000)  # can be increased
...
if __name__ == '__main__':
    ...
    guidance = StableDiffusion(
        ...
        min=args.sd_min,
        max=args.sd_max,
        local_files_only=args.sd_local_files_only,
        cache_dir=args.sd_cache_dir,
        use_auth_token=hf_token,
        allow_fallback=not args.disable_sd_fallback,
    )
    guidance.eval()
    for p in guidance.parameters():
        p.requires_grad = False

    args.exp_name = args.caption.replace(' ', '_')
    args.objaverse_id = 'none'
    args.identity = args.caption
    main(args, guidance)
